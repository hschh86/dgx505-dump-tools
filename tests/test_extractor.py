...
@pytest.fixture(scope='module')
def ffcd():
    ffc = e._read_dump_from_filename('test_data/dumptestpartial.syx', songonly=True)
    ffd = e._read_dump_from_filename('test_data/dumptestpartial.txt', songonly=True)
    return ffc, ffd
...
def test_incomplete():
    with pytest.raises(e.MessageSequenceError):
...
def test_midi(ffab, ffcd):
    ...
    with pytest.raises(e.NotRecordedError):
